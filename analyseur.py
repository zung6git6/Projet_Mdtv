...
def json_file(list_elements: list, output_file: str) -> None:
    ...
    # Fonction utilitaire pour standardiser les conditions
    def clean_condition(condition):
        return condition.replace(" ", "")
    
    # Récupération du premier symbole pour vérifier la validité initiale
    symbol0 = list_elements[0]
    context_stack = [structure_syntaxique]  # Pile de contextes, initialisée avec la structure principale
    current_block = context_stack[-1]

    def ajouter_instruction(type_, instruction, suivant=None, contenu=None):
    ...
    if symbol0 not in not_q0_symbols:
        ...
        for i, symbol in enumerate(list_elements):
            ...
            # Traitement des conditions (si(0), si(1))
                ...
                condition_block = []
                ajouter_instruction("condition", symbol, suivant=next_symbol, contenu=condition_block)
                context_stack.append(condition_block)  # Empile le nouveau bloc de condition
                current_block = context_stack[-1]  # Change le contexte

            # Traitement des boucles
            elif symbol == 'boucle':
                boucle_block = []
                ajouter_instruction("boucle", symbol, suivant=next_symbol, contenu=boucle_block)
            ...
            # Traitement des fermetures
                ...
                if context_stack:  # Vérifie qu'il reste des contextes à sortir
                    ...
                    if context_stack:
                        current_block = context_stack[-1]  # Remonte au contexte précédent
